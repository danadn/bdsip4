...
public class VentanaMuestra extends javax.swing.JFrame {
    ...
    private void rellenaComboEleccion(estados s) throws SQLException {
        if (manejador.getId() != null) {
            int idDoc = (int) Integer.valueOf(manejador.getId());
            String consulta = "";
            if (s == estados.CREAR) {
                switch (manejador.getEstado()) {
                    case BBDD_PERSONAJE:
                        consulta = manejador.getBBDDManager().consultaPeticion("SELECT * FROM personaje WHERE nombreYApe NOT IN (SELECT personaje FROM personajesdocumento WHERE idDocumento=" + idDoc + ");","nombreYApe");//   SELECT * FROM personajesdocumento WHERE idDocumento<>" + idDoc + ";", "personaje");
                        break;
                    case BBDD_ACONTECIMIENTO:
                        consulta = manejador.getBBDDManager().consultaPeticion("SELECT * FROM acontecimiento WHERE nombreAcont NOT IN (SELECT acontecimiento FROM acontecimientosdocumento WHERE idDocumento=" + idDoc +");","nombreAcont");//SELECT * FROM acontecimientosdocumento WHERE idDocumento<>" + idDoc + ";", "acontecimiento");
                        break;
                    case BBDD_COLECTIVO:
                        consulta = manejador.getBBDDManager().consultaPeticion("SELECT * FROM colectivo WHERE nombre NOT IN (SELECT colectivo FROM colectivosdocumento WHERE idDocumento=" + idDoc + ");","nombre");//SELECT * FROM colectivosdocumento WHERE idDocumento<>" + idDoc + ";", "colectivo");
                        break;
                }
                tablaEncontrados.removeAll();
                DefaultTableModel m = new DefaultTableModel(new Object[]{"Elementos encontrados"}, 0);
                if (consulta != null) {
                    String[] con = consulta.split(",");
                    for (int k = 0; k < con.length; k++) {
                        m.addRow(new Object[]{con[k]});
                    }
                }
                ...
                switch (manejador.getEstado()) {
                    case BBDD_PERSONAJE:
                        consulta = manejador.getBBDDManager().consultaPeticion("SELECT * FROM personajesdocumento WHERE idDocumento=" + idDoc + ";", "personaje");
                        break;
                    case BBDD_ACONTECIMIENTO:
                        consulta = manejador.getBBDDManager().consultaPeticion("SELECT * FROM acontecimientosdocumento WHERE idDocumento=" + idDoc + ";", "acontecimiento");
                        break;
                    case BBDD_COLECTIVO:
                        consulta = manejador.getBBDDManager().consultaPeticion("SELECT * FROM colectivosdocumento WHERE idDocumento=" + idDoc + ";", "colectivo");
                        break;
                }
                tablaEncontrados.removeAll();
                DefaultTableModel m = new DefaultTableModel(new Object[]{"Elementos encontrados"}, 0);
                ...
                tablaEncontrados.setModel(m);
                //consulta=manejador.getBBDDManager().consultaPeticion("", "");
            }

        } else if (s == estados.CREAR || s == estados.MODIFICAR) {
            String consulta = "SELECT * FROM ";
            String campo = "";
            switch (manejador.getEstado()) {
                case BBDD_PERSONAJE:
